...
export async function handler(event: APIGatewayEvent, context: Context): Promise<ResponseBody> {
  ...
  const session: ResultType<UserSession> = await getSession(cookieResult.data.session_id, client)

  if (session.Status === Status.Err) {
    return errorResponse({ tag: "SessionNotFound" })
  }

  const sessionDestroyed: boolean = await destroySession(session.data.sessionId, client)
