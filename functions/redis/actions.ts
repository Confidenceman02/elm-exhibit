...
import {ExpirableDBTag, IPromisifiedRedis, PermanentDBTag, TempSession} from "./types";
...
export async function initTempSession(meta: TempSession, client: IPromisifiedRedis): Promise<boolean> {
...
export async function initSession(sessionId: string, gitUser: GithubUserData, client: IPromisifiedRedis): Promise<boolean> {
...
export async function getSession(sessionId: string, client: IPromisifiedRedis): Promise<ResultType<UserSession>> {
...
export async function sessionExists(sessionId: string, client: IPromisifiedRedis): Promise<boolean> {
  const dbKey = generateExpirableDBKey(ExpirableDBTag.Session, sessionId)
  const keyExists = await client.EXISTSAsync(dbKey)
  return !!keyExists
}

export async function createUser(gitUser: GithubUserData, client: IPromisifiedRedis): Promise<boolean> {
